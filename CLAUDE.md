...
- `uv run python manage.py seed_store` — idempotent: seeds the store catalog. Perks are still placeholder data (real names/costs for Cat/Mouse's own perks aren't designed yet), but all three characters' outfit cosmetics are now real: dog's `DOG_OUTFIT_ITEMS` (4 of 5 planned tutu colors — pink/teal/orange/green — point at real per-color walk-cycle art, `is_active=True`; purple stays a placeholder, `is_active=False`, pointing at the default `dog_v2.png`, pending its own art regeneration), cat's `CAT_OUTFIT_ITEMS` (teal/orange/green/purple — pink skipped, already the default look) and mouse's `MOUSE_OUTFIT_ITEMS` (pink/teal/orange/purple — green skipped, already the default look) — see the sibling `kat_trap` repo's CLAUDE.md, Currency & economy system section, for that art's own extraction history. `PET_SHOP_ITEMS` (added 2026-09-16) seeds the Pet Shop Boys decor catalog — `item_type=ItemType.DECOR`, `character=''` (see "Pet Shop Boys" below) — with placeholder flat-shape art; `hamster-cage` ships `is_active=False` pending its own placement wiring on the `kat_trap` side.
...
`StoreItem(character, slug, name, item_type[cosmetic|perk|decor], cost, min_level)` is a per-character catalog — its `character` field is both "which wallet pays for this" and "which wallet's level gates it" — **except for `item_type=decor` rows, where `character` is blank** (see "Pet Shop Boys" below: decor isn't any one character's). `OwnedItem(user, item)` is a permanent purchase record (`unique_together` blocks re-buying the same item) — note this was already character-less before decor existed, which is exactly what let decor's "global ownership" fall out for free. `sell_item()` (`kattrap/services.py`) is the reverse: refunds `SELL_REFUND_FRACTION` (0.2, rounded to a whole coin — see `economy.py`) of `cost` and deletes the `OwnedItem` row; deliberately well under 1.0 so buy-then-sell can't launder coins. Only cosmetics and perks alike can be sold — there's no item-type restriction, unlike equip below.

### Pet Shop Boys: decor, a third `item_type` with no character of its own

Added 2026-09-16 for `kat_trap`'s new decor store (kitchen fish tank/bird cage/etc.) — decor-only today, no gameplay effect (a future perk-like "effect" field is meant to layer onto this same modeling later, not replace it). `ItemType.DECOR` alongside `cosmetic`/`perk`; `StoreItem.character` is `blank=True` specifically to allow `''` for these rows (unique_together still holds — decor slugs just need to be unique among themselves, same as any one character's own slugs). `OwnedItem`'s pre-existing `(user, item)` key (no character column) meant global ownership needed **zero schema change** beyond that — see the model's own docstring for why this wasn't decor-specific design, just a happy consequence of an existing choice.

`purchase_decor_item()`/`sell_decor_item()` (`kattrap/services.py`) share their actual debit/refund logic with `purchase_item()`/`sell_item()` via `_apply_purchase()`/`_apply_sale()` helpers — the two pairs differ only in how `item` gets looked up (`character=character` vs. `item_type=ItemType.DECOR`, no character filter). `character` in `purchase_decor_item(user, character, item_slug)` means only "whose wallet pays" — decor items don't have their own character to match against. `DecorStoreView`/`DecorPurchaseView`/`DecorSellView` (`kattrap/views.py`) live at `/decor/`, `/decor/purchase/`, `/decor/sell/` (not under `/store/<character>/`) — `DecorItemActionSerializer` carries `character` in the request body instead of the URL, since these routes aren't per-character. No equip/unequip endpoints exist for decor — it's never equipped, only owned.
...
- **`kat_trap` is fully wired up to this API** — wallet/HUD, the Bloomingtails (cosmetics) + Pawgreens (perks) + Pet Shop Boys (decor) store, purchase/sell/equip/unequip, and round submission all call real endpoints here. `kpground` (the landing page) still has no integration with this API at all — shared account/auth across the whole site is still just the plan, not built.
- **Pet Shop Boys' `hamster-cage` row ships `is_active=False`** — seeded and real, but `kat_trap`'s counter-substitution placement isn't wired up for it yet (see that repo's own CLAUDE.md). Flip once that's built; no backend change needed.
